...
- **밝은 계열 글래스모피즘 테마 (`utils/ui_theme.py`, `.streamlit/config.toml`)**:
  - 소프트 펄 화이트/스카이블루 파동 배경(`assets/background.jpg`) 위에 흰색 그라데이션 베일을 얹어 콘텐츠 가독성 확보.
  - `config.toml`에서 Pretendard 폰트, 인디고 포인트 컬러(`#4f46e5`), 둥근 모서리(`baseRadius`/`buttonRadius`), 축소된 제목 크기(`headingFontSizes`)를 네이티브 테마 옵션으로 지정.
  - `key="card_*"`로 선언한 `st.container`는 흰색 반투명 카드(둥근 20px 모서리 + 소프트 섀도)로 자동 스타일링 (Streamlit 1.64의 `st-key-*` 클래스 활용).
  - 공통 컴포넌트: `page_header(icon, title, subtitle, chips)` — 아이콘 타일 + 제목 + 상태 칩(모델명, 메시지 수, Key 상태 등) / `empty_state(title, subtitle)` — 로고 + 안내 문구 중앙 정렬 빈 상태 블록.
- **채팅 말풍선 UI**:
  - 사용자 메시지는 우측 정렬 인디고 그라데이션 버블, AI 답변은 좌측 정렬 화이트 버블(CSS `:has()`로 아바타 종류에 따라 분기).
  - AI 아바타는 브랜드 로고(`assets/logo.jpg`), 사용자 아바타는 인디고 배경.
  - 채팅 입력창은 둥근 16px 카드 형태에 포커스 시 인디고 링, 하단 영역은 흰색 페이드 그라데이션.
- **상단 수평 네비게이션 (`position="top"`)**:
  - Streamlit 상단 네비게이션바를 알약(pill) 형태로 스타일링, 현재 페이지는 인디고 틴트로 강조. Deploy 버튼은 숨김.
  - 로그인 상태가 상단 `계정 ({user_id})` 탭에 자연스럽게 표시됨.
- **네비게이션 로고 브랜딩 (`st.logo`, `size="large"`)**:
  - 사이드바에는 워드마크 배너(`assets/sidebar_banner.jpg`, 여백 크롭본), 사이드바가 없는 화면의 헤더에는 아이콘(`assets/logo.jpg`) 표시.
- **사이드바 역할 분리**:
  - 사이드바에는 전역 로그인 정보나 계정 관련 위젯을 일절 배치하지 않음.
  - 각 페이지별 고유 옵션(예: 채팅 페이지의 모델/세션 선택, 히스토리 페이지의 대화 탐색) 전용 공간으로 독립 운영.
  - 대화 목록은 `st.radio`를 리스트 형태로 스타일링(라디오 원형 숨김, 선택 항목 인디고 하이라이트). `session_labels()`로 동일 제목에 ` (2)`, ` (3)`을 붙여 라벨 중복 시 선택 표시가 어긋나는 문제를 방지.
...
| 상태 | 기능 항목 | 상세 명세 |
| :--- | :--- | :--- |
| **비로그인** | **중앙 로그인 카드** | • 3D AI 어시스턴트 일러스트(`assets/auth_hero.jpg`) + "환영합니다 👋" 헤드라인 + 안내 문구<br>• 아이콘이 붙은 사용자 ID 입력창 및 로그인 버튼 (ID 기반 세션 격리)<br>• 카드 하단에 비밀번호 없는 ID 로그인 안내 캡션 |
| **로그인 완료** | **계정 정보 & 바로가기** | • 좌측 카드: AI 일러스트, 로그인 계정 / API Key 상태 메트릭(아이콘 포함)<br>• 우측 카드: 채팅 · API Key 관리 · 대화 내역 바로가기 버튼<br>• 세션 종료 카드: 로그아웃(브라우저 보관 Key 동시 삭제) |
...
| 기능 항목 | 상세 명세 |
| :--- | :--- |
| **페이지 헤더** | • `page_header`에 Key 상태 칩(`등록됨`/`미등록`) 표시 |
| **Key 등록/상태 카드** | • 좌측 보안 쉴드/키 일러스트(`assets/security_key.jpg`) + 우측 폼의 분할 카드<br>• 미등록: 패스워드 마스킹 입력 필드 및 `[Key 등록하기]` 버튼<br>• 등록 시 마스킹 코드 블록(`sk-...XXXX`), `[채팅 시작]`, `[Key 삭제]` 버튼 |
| **보안 안내 카드** | • DB 미저장, 브라우저 세션 메모리 보관, 공용 PC 주의 수칙 안내 및 OpenAI 플랫폼 바로가기 링크 버튼 |
...
| 기능 항목 | 상세 명세 |
| :--- | :--- |
| **Key 미등록 안내** | • 로고 + "API Key가 아직 없어요" 빈 상태 카드와 `[API Key 등록하러 가기]` 버튼 |
| **페이지 헤더** | • 현재 대화 제목 + 적용 모델 칩 + 메시지 수 칩 |
| **고정 높이 스크롤 뷰포트** | • `st.container(height=540, border=True, key="card_chat")` 카드 안에서 대화가 길어져도 상단 헤더가 밀리지 않고 내부에서 부드럽게 스크롤<br>• 빈 대화 시 로고와 친근한 온보딩 안내(`empty_state`) 표시, 첫 메시지 전송 시 placeholder 제거 |
| **채팅 말풍선** | • 사용자(우측, 인디고) / AI(좌측, 화이트 + 로고 아바타) 버블로 구분 |
| **채팅 입력창 도킹** | • 스크롤 채팅창 바로 아래에 둥근 카드형 `st.chat_input`을 안정적으로 고정 배치 |
| **사이드바 컨트롤** | • 상단 **`[+ 새 대화]`** 버튼<br>• 모델 선택 (`gpt-5.6-luna` 기본)<br>• 대화 목록 리스트(라디오, `on_change` 콜백으로 세션 전환)<br>• `[대화 관리]` 익스팬더 안에 현재 대화 삭제 및 모든 대화 초기화 버튼 |
...
| 기능 항목 | 상세 명세 |
| :--- | :--- |
| **빈 상태** | • 저장된 대화가 없으면 로고 + 안내 카드와 `[지금 대화 시작하기]` 버튼 |
| **사이드바 세션 탐색** | • 리스트 형태의 대화 목록(라디오) 및 선택 대화 삭제 버튼(삭제 시 위젯 선택값 초기화) |
| **페이지 헤더** | • 선택 대화 제목 + 생성 일시 칩 + 메시지 수 칩 |
| **💬 대화 내용 탭** | • `st.container(height=480, border=True, key="card_history_chat")` 내부 스크롤 뷰(좌우 말풍선 + 타임스탬프 캡션), `[텍스트 파일 (.txt) 다운로드]` |
