...
- **모던 UI 테마 (`utils/ui_theme.py`)**:
  - Pretendard 한글 폰트 스택, 카드 음영, 반응형 여백, 호버 인터랙션 적용.
  - 상태 칩(Status Chip) 및 글래스모피즘 카드 컴포넌트 제공.
- **로그인 분기 라우팅**:
  - `st.session_state["user_id"]` 미존재 시: 중앙 집중형 **`👤 로그인`** 페이지만 사이드바에 노출.
  - 로그인 성공 시: **`💬 대화 서비스`**, **`📜 데이터 관리`**, **`👤 사용자 계정`** 그룹 메뉴 활성화.
- **사이드바 글로벌 컨트롤**:
  - 브랜드 배너 (AI Studio) 및 온라인 상태 인디케이터
  - 접속자 ID 및 API Key 실시간 등록 상태 뱃지 (`🟢 Key 활성화` / `⚠️ Key 미등록`)
  - **`[🚪 로그아웃]` 버튼**: 메모리에 존재하는 `user_id`, `openai_api_key`, `current_session_id`를 즉시 영구 파기하고 초기 화면으로 복귀.

---

### 3.2. 👤 로그인 & 프로필 대시보드 (`pages_app2/auth.py`)
| 상태 | 기능 항목 | 상세 명세 |
| :--- | :--- | :--- |
| **비로그인** | **중앙 히어로 로그인 카드** | • 사용자 ID/닉네임 입력으로 즉각 세션 격리 생성<br>• 핵심 강점 3종(Zero DB 저장, 스마트 세션, GPT-5.6-Luna) 카드 배치 |
| **로그인 완료** | **프로필 대시보드** | • 환영 배너 및 온라인 상태 뱃지<br>• 활성 세션(N/10), 누적 메시지 수, API Key 상태의 3대 메트릭 요약<br>• 챗봇/Key설정/히스토리로 바로 이동하는 3종 원클릭 바로가기 카드<br>• 안전한 로그아웃 제어 |

---

### 3.3. 🔑 API Key 등록 및 보안 센터 (`pages_app2/key_settings.py`)
| 기능 항목 | 상세 명세 |
| :--- | :--- |
| **보안 가이드 4종 그리드** | • **Zero-Storage**: DB/디스크 미저장 원칙<br>• **세션 휘발성**: 탭 종료/로그아웃 시 메모리 소멸 원칙<br>• **공용 PC 수칙**: 공용 환경 키 파기 권고<br>• **엔드포인트 직결**: OpenAI 공식 SSL 엔드포인트 직결 원칙 |
| **Key 상태 카드 & 원클릭 이동** | • 등록 시: 마스킹 키(`sk-••••XXXX`) 표기 및 `[💬 지금 챗봇과 대화하러 가기]` 원클릭 이동 지원<br>• `[🗑️ API Key 즉시 파기]` 버튼으로 메모리에서 즉시 파기 |
| **Key 등록 폼** | • 미등록 시: 패스워드 마스킹 필드 및 OpenAI 플랫폼 키 발급 바로가기 링크 제공 |
...
| 기능 항목 | 상세 명세 |
| :--- | :--- |
| **상단 헤더 바** | • 현재 세션 제목, 선택 모델(`gpt-5.6-luna`), 현재 메시지 용량(`N / 200 msgs`) 뱃지 실시간 표시<br>• **`[✏️ 대화 제목 수정]`** 팝오버를 통해 언제든지 방 제목 즉시 변경 가능 |
| **사이드바 세션 게이지** | • 세션 보관 현황 프로그레스 바(0~100%) 시각화<br>• `[➕ 새 대화 시작]` 및 드롭다운 세션 전환, 개별 세션/전체 대화 영구 삭제 |
| **온보딩 프롬프트 칩** | • 새 세션 시작 시 빈 화면 대신 환영 인사 및 3가지 추천 질문 카드 제공 (클릭 시 자동 전송) |
| **실시간 스트리밍 대화** | • `gpt-5.6-luna` 기반 OpenAI 실시간 타이핑 스트리밍 (`st.write_stream`)<br>• 이미지/파일 업로드 배제 순수 텍스트 대화 |
| **수명 주기 자동화 (FIFO)** | • 세션당 100회(200개 메시지) FIFO 유지, 유저당 최대 10개 세션 FIFO 유지 |

---

### 3.5. 📜 대화 기록 보관소 (`pages_app2/history.py`)
| 기능 항목 | 상세 명세 |
| :--- | :--- |
| **상단 요약 메트릭 카드** | • 보관 세션 현황(N/10), 선택 세션 메시지 수(N/200), 세션 생성 시각, 소유 계정의 4개 지표 카드 |
| **사이드바 실시간 필터** | • 세션 제목 실시간 검색 및 라디오 버튼 기반 직관적 세션 탐색 |
| **💬 타임라인 대화 뷰** | • 실제 챗 버블 및 발송 시각 타임스탬프 뱃지 렌더링<br>• **`[📥 대화 내역 텍스트 다운로드 (.txt)]`** 버튼 제공 |
| **📊 데이터 테이블 뷰** | • 원본 데이터프레임 구조를 100% 보존하면서 `st.column_config`로 열 너비 및 일시 포맷 최적화<br>• **`[📥 CSV 파일로 내보내기 (.csv)]`** (UTF-8 BOM) 버튼 제공 |
