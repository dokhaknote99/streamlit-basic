...
- **글로벌 UI 테마 (`utils/ui_theme.py`)**:
  - `assets/background.jpg` 배경 이미지를 색상 덮어씌움 없이 선명하게 전체 화면에 노출 (`stAppViewContainer`).
  - Pretendard 폰트 스택 및 가독성을 위한 컨테이너 반투명 글래스 스타일 적용.
- **플랫한 내비게이션 (목차 제거)**:
  - 사이드바 내 불필요한 카테고리/목차 헤더를 제거하고 **`채팅`**, **`API Key 등록`**, **`대화 내역`**, **`계정`**의 직관적인 플랫 목록으로 구성.
- **사이드바 사용자 정보**:
  - 접속자 ID, API Key 상태(`등록됨`/`미등록`), 로그아웃 버튼 배치.
...
| 상태 | 기능 항목 | 상세 명세 |
| :--- | :--- | :--- |
| **비로그인** | **로그인 폼** | • 사용자 ID 입력창 및 로그인 버튼 (ID 기반 세션 격리) |
| **로그인 완료** | **계정 상태 및 로그아웃** | • 현재 로그인 사용자명, Key 상태 표시 및 로그아웃 버튼 |
...
| 기능 항목 | 상세 명세 |
| :--- | :--- |
| **보안 안내** | • DB 미저장, 브라우저 세션 메모리 보관, 공용 PC 주의, 키 노출 방지의 4대 수칙 안내 |
| **Key 등록/삭제** | • 패스워드 마스킹 입력 필드 및 등록 버튼<br>• 등록 시 마스킹 상태 표시(`sk-...XXXX`) 및 `[API Key 삭제]` 버튼 |
...
| 기능 항목 | 상세 명세 |
| :--- | :--- |
| **사이드바 컨트롤** | • 상단 **`[➕ 새 대화]`** 버튼<br>• 모델 선택 (`gpt-5.6-luna` 기본)<br>• 대화 세션 목록 선택 셀렉트박스<br>• 현재 대화 삭제 및 모든 대화 초기화 버튼 |
| **실시간 스트리밍 대화** | • OpenAI 실시간 타이핑 스트리밍 (`st.write_stream`)<br>• 불필요한 장식(프롬프트 추천 칩, 진행바 등)을 배제한 순수 대화창 |
...
| **💬 대화 내용 탭** | • 메시지 버블 및 기록 시각 표시, `[텍스트 다운로드 (.txt)]` |
| **📊 데이터 테이블 탭** | • 원본 데이터프레임 구조를 100% 보존하며 `st.dataframe(width="stretch")` 출력, `[CSV 다운로드 (.csv)]` |
